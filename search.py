...
def searchTags(query):
    ...
    timestamp = int(time.time())
    method = "GET"
    url = f'https://api.letterboxd.com/api/v0/search?include=TagSearchItem&perPage=10&searchMethod=FullText&input={query}&apikey={apikey}&nonce={nonce}&timestamp={timestamp}'
    bytestring = b"\x00".join(
        [str.encode(method), str.encode(url), str.encode("")]
    )
    signature = hmac.new(
    ...
    r = requests.get(f'https://api.letterboxd.com/api/v0/search?include=TagSearchItem&perPage=10&searchMethod=FullText&input={query}&apikey={apikey}&nonce={nonce}&timestamp={timestamp}&signature={signature}', verify=False)
    resultJSON = r.json()
    return resultJSON
